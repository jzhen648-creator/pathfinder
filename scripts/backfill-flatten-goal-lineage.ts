/**
 * Clear Goal.parentGoalId — pursuits are peers within a category (no nest tree).
 * Also migrates any legacy ON_HOLD rows to PAUSED (pre-enum migration stragglers).
 *
...
async function main() {
  ...
  if (legacyOnHold > 0) {
    const migrated = await prisma.$executeRaw`
      UPDATE "Goal"
      SET "status" = 'PAUSED'::"PursuitStatus"
      WHERE "status"::text = 'ON_HOLD'
    `;
    console.log(`Migrated ${migrated} goal(s) from ON_HOLD → PAUSED.`);
  } else {
    console.log("No ON_HOLD status rows found.");
  }
