...
- **Cinematic intro video (post-stability):** 30–60s AI-generated video for App Store preview, onboarding splash, and marketing (website hero, social). Emotional priming, not a feature walkthrough: problem (insights disappearing) → a path appears → map comes alive → "Start talking." Visual language: dark background, thin strokes, warm cream nodes (Almanac aesthetic in cinematic form). Tools to explore: Sora, Kling, Runway. **Status:** Not started — do not build until core product is stable and in users' hands. Detail: [`../DECISIONS.md`](../DECISIONS.md#backlog--future-ideas).
