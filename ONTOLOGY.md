...
| **Timeline note** | Prisma `Mark` | Hub-scoped only (`branchId`); **not** nested under a pursuit. UI: `MarkHoverCard` on tree + hub panel **Add mark**. `Mark.kind` ∈ {`mark`, `stream`}. `needsResolution` = Stream ambiguous item awaiting user resolution on map. |
