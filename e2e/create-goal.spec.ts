...
test.describe("Create goal from Tasks", () => {
  ...
  test("opens modal, submits, and shows the new goal", async ({ page }) => {
    ...
    await page.waitForURL(/\/(dashboard|tasks|onboarding|tree)/, { timeout: 30_000 });
    if (page.url().includes("/onboarding")) {
      test.skip(true, "E2E user must complete onboarding first");
    }

    await page.goto("/tasks");
    await page.waitForURL(/\/(tasks|login|onboarding)/, { timeout: 15_000 });
    if (page.url().includes("login")) {
      test.skip(true, "Session not established after sign-in");
    }
    if (!page.url().includes("tasks")) {
      test.skip(true, "Redirected away from Tasks");
    }
    ...
    await page.goto("/tasks");
    await expect(page.locator('[data-testid="tasks-add-goal"]')).toBeVisible({ timeout: 20_000 });

    await page.locator('[data-testid="tasks-add-goal"]').click();
