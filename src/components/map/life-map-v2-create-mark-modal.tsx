...
export function LifeMapV2CreateMarkModal({
  open,
  onClose,
  branches,
  defaultLifeAreaId,
  ...
  const submit = useCallback(async () => {
    if (!branchId) {
      setError("Pick a thread or start one in Life Map first.");
      return;
    }
    const t = title.trim();
    if (!t) {
      ...
      >
        ...
        <div className="flex flex-col gap-3 text-sm">
          <label className="flex flex-col gap-1">
            <span className="text-[var(--rm-text3,#6B7280)]">Life area</span>
            <select
          ...
          <label className="flex flex-col gap-1">
            <span className="text-[var(--rm-text3,#6B7280)]">Thread</span>
            <select
              className="rounded-lg border px-2 py-2"
              style={{ borderColor: "var(--rm-border)", color: "var(--rm-text1)" }}
              value={branchId}
