...
export const PURSUIT_STATUS_PROMPT_LINES = [
  "Chapter status (status field in map context):",
  "- COMPLETE — the reader already sees Complete in the meta strip. Headline names what finishing changes or unlocks on the map (a sibling chapter it feeds, a tension that clears, a frontier that opens) — not that the chapter is done, and not character commentary about the person. Treat completed chapters as settled facts, not gaps or nudges.",
  ...
  "- PAUSED — the chapter is deliberately paused. Reflect why the pause may be intentional or what is waiting — warm, no pressure to resume. Do not treat as a gap, failure, or nudge to unpause.",
] as const;
