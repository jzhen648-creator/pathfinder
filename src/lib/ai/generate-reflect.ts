...
const HEADLINE_MUST_ADD_MEANING = [
  ...
  '- Never restate the status line ("X is paused with a deadline of Y"), the milestone count ("X has N milestones complete"),',
  '  or raw days-until-deadline ("Deadline in 716 days") — the meta strip, Milestones tab, and Timeline already own those.',
  "- Milestones are optional scaffolding — they do not measure completion. Do not treat N of M as % done.",
  "- The user already sees those. Meaning = useful specificity the UI does not show — authored fact, amount frontier,",
  "  chronology, conflict, or an urgent deadline paired with another concrete frontier.",
  "- Meaning is NOT a slogan, riddle, metaphor closer (\"the story\", \"anchor\", \"through-line\"), or administrative audit.",
  ...
  '- Wrong: "The training gap is the story"',
  '- Right: "5k is done — the jump to 10k is where the training plan actually starts"',
  "- If there's nothing meaningful to add beyond status/plan inventory, write \"\" rather than padding with day counts or milestone ratios.",
].join("\n");
...
const REFLECT_BENCHMARK_INSIGHT_RUBRIC = [
  ...
  "location, or stage? State the fact and any benchmark contrast — do not predict outcomes.",
  ...
  "  map are a fraction of that.",
  '- Strong: "Contributions hit the full £20k allowance — that is uncommon; most people use a fraction."',
  "",
  "Move 2 — Read the combination. What contrast exists because these two things sit on the map together?",
  ...
  "(deadline, target, milestone frontier). State both facts plainly — do not judge pace,",
  "predict success, describe consequences, or end with a riddle.",
  '- Weak: "Your half-marathon training is in progress."',
  '- Weak: "The race gap is the story."',
  '- Strong: "The race is ten weeks out and the longest logged run is still 8k."',
  "",
  ...
  "lived pause (pursuit_paused marks where a pause began). A return is a map fact — name it",
