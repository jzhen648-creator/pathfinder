...
const STREAM_SELF_MIND_THEME_BOUNDARIES = [
  ...
  "",
  "Hub slugs (normalized — use exactly as in hub context): values & direction | mind & wellbeing",
  "",
  "Values & direction: life direction, meaning, identity, principles, values, self-definition, long-term personal direction.",
  "Mind & wellbeing: thoughts, feelings, mental patterns, emotional regulation, confidence, anxiety, resilience, self-talk, journaling, reflection, motivation, discipline, procrastination, therapy, psychological wellbeing.",
  "",
  ...
  "- Assets, debt, investing, property, mortgages, savings, tax, income → Money & Finance (finance theme: pay from work | property income | business & freelance | assets & investing | safety net & insurance | debts & loans)",
  "",
  "Social fun with other people may → Friends & community or Partner & romance even when the activity is leisure.",
...
const STREAM_PLAY_LEISURE_THEME_BOUNDARIES = [
  ...
  "",
  "Hub slugs: hobbies & making | books, film & culture | trips & events",
  "",
  "Hobbies & making: personal interests, crafts, gaming, collecting, gardening, making, tinkering, learning an instrument for fun, sport done for joy.",
  "Books, film & culture: books, reading, music, art, film, theatre, concerts, media consumption and appreciation.",
  "Trips & events: travel, trips, holidays, festivals, events, adventures, purchases made for joy (including collector cars for driving pleasure).",
  "",
  ...
  "- Income, spending plans, investing → Money & Finance (split income by source: salary → pay from work; rental/landlord → property income; freelance/business → business & freelance)",
  "",
  "When unsure between Hobbies & making vs Books, film & culture: making/playing → hobbies & making; reading/watching/listening → books, film & culture; planning a trip/event → trips & events.",
].join("\n");
...
export const STREAM_EXTRACT_THEME_SYSTEM_PROMPT = [
  ...
  "",
  "- Every mark, pursuit, and milestone MUST include categorySlug — the normalized category slug from context (e.g. \"jobs & roles\", \"qualifications\", \"education & courses\", \"projects & launches\", \"career search\" for Work & Career; \"pay from work\", \"property income\" for Money & Finance).",
  "- Never use display labels (\"Jobs & roles\", \"Pay from work\") as categorySlug — only the slug.",
  "- Route each item to the category whose catalog scope and AI routing note best fit the user's intent.",
  "- Work & Career examples: promotions, roles, pivots → jobs & roles; named licenses and certifications → qualifications; courses, degrees, deliberate practice → education & courses; shipping, portfolios, concrete deliverables → projects & launches; active job hunt → career search.",
  "- Money & Finance examples: salary, bonus, employer pay → pay from work; BTL, landlord, tenants, rental yield → property income; freelance, invoicing, business revenue → business & freelance; ISA, pension, investing → assets & investing.",
...
export const STREAM_EXTRACT_GLOBAL_SYSTEM_PROMPT = [
  ...
  STREAM_PLAY_LEISURE_THEME_BOUNDARIES,
  "",
  "## Values & direction vs Jobs & roles category routing",
  "",
  "Moral/spiritual identity language → Values & direction hub (becoming / Self & Mind theme), even when the paragraph also mentions politics or law:",
  '- "keep the ideal alive", moral core, dignity, hope, forgiveness, reconciliation, legacy, conscience → values & direction',
  "",
  "Career/work language → Jobs & roles category (work theme):",
  '- legal practice, law firm, attorney, ANC leadership, election campaign, voter education → jobs & roles',
  "",
  STREAM_EXTRACT_TITLE_LENGTH_RULES,
  "",
  "Creation rules:",
