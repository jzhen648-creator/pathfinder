...
export const OVERALL_READING_FIELD_JOB = [
  ...
  "- oneLiner: one complete sentence (<= 140 chars) naming the 2–3 heaviest active chapters across themes.",
  ...
  "- support: one supporting sentence (<= 480 chars) — a whole-map state-of-play naming every theme",
  "  in mapAggregates.themeRollup. For each theme, cite its headline chapter and in-progress count",
  "  when relevant; weave themes together — do NOT repeat the hero's conclusion.",
  ...
  '- Right support: "Money & Finance centres on the ISA target; Work & Career on the apprenticeship with one milestone done; People & Relationships holds the wedding plan."',
].join("\n");
...
export const OVERALL_READING_OUTPUT_CONTRACT = [
  ...
  "  tone MUST be one of: celebratory | encouraging | nudge",
  "  oneLiner <= 140 chars — whole-map hero; support <= 480 chars — panoramic per-theme state-of-play.",
