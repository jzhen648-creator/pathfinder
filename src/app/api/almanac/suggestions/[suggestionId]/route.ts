...
import { mutateAlmanacSuggestionRequestSchema } from "@/lib/almanac/contracts";
import { almanacDogfoodEnabled } from "@/lib/almanac/feature";
import { almanacRouteError } from "@/lib/almanac/route-response";
import { mutateAlmanacSuggestion } from "@/lib/almanac/suggestion-service";
...
export async function PATCH(
  ...
  const parsed = mutateAlmanacSuggestionRequestSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    return NextResponse.json(await mutateAlmanacSuggestion(auth.userId, suggestionId, parsed.data));
  } catch (error) {
