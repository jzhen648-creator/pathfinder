...
import { acceptAlmanacSuggestionRequestSchema } from "@/lib/almanac/contracts";
import { almanacDogfoodEnabled } from "@/lib/almanac/feature";
import { almanacRouteError } from "@/lib/almanac/route-response";
import { acceptAlmanacSuggestion } from "@/lib/almanac/suggestion-service";
...
export async function POST(
  ...
  const parsed = acceptAlmanacSuggestionRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      { error: parsed.error.issues[0]?.message ?? "Invalid acceptance request." },
      { status: 400 },
    );
  }
  try {
    ...
    return NextResponse.json(
      await acceptAlmanacSuggestion(auth.userId, suggestionId, parsed.data),
    );
  } catch (error) {
